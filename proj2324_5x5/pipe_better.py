    ...
    def h(self, node: Node):
        ...
        for row in range(node.state.board.dimension()):
            for col in range(node.state.board.dimension()):
                for delta_row, delta_col in flow_delta[node.state.board.get_value(row, col)]:
                    possible += 1
                    new_row = row + delta_row
                    new_col = col + delta_col
                    if 0 <= new_row <= node.state.board.dimension() - 1 and 0 <= new_col <= node.state.board.dimension() - 1:
                        for new_delta_row, new_delta_col in flow_delta[node.state.board.get_value(new_row, new_col)]:
                            if row == new_row + new_delta_row and col == new_col + new_delta_col:
                                connected += 1
